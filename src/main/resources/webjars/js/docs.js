...
var tempBodyResponseData = $.templates('#temp_body_response_data');
var tempBodyResponseCode = $.templates('#temp_body_response_code');
            ...
            $("[name='a_path']").click(function () {
                ...
                $.each(jsonData.paths[path], function (i, d) {
                    if (d.operationId == operationId) {
                        ...
                        var dataName = getResponseModelName(d.responses["-200"]["schema"]["$ref"]);
                        if (dataName) {
                            $("#path-body-response-data").html(tempBodyResponseData.render(jsonData.definitions[dataName]));
                        }
                        $("#path-body-response-code").html(tempBodyResponseCode.render(d));
                    }
                });
            });
